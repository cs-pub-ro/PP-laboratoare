...
(check-exp-part 'a .5 (rev '(5 1 4 8 7)) '(7 8 4 1 5))
(check-exp-part 'b .5 (rev '(a b c a)) '(a c b a))
...
(check-exp-part 'b .5 (first-b-pal 154 4) 373)
(check-exp-part 'c .5 (first-b-pal 154 3) 154)
...
(define (longest-palindrome n)
  ...
  (define (iter L len crtlen) ;; caută în L palindroame de lungime len (crtlen = lungime curentă L) ;; întoarce palindromul dacă există, altfel #f
    (and (<= len crtlen)                              ;; observație: (and #t 5) întoarce 5 ; (or #f 4) întoarce 4 (se putea și cu if-uri)
         (if (palindrome? (take L len))
             (list->num (take L len))                 ;; întoarcem palindromul ca număr, nu ca listă
